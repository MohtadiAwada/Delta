package dev.moti.delta.commands;

import dev.moti.delta.Delta;
import java.util.ArrayList;
import java.util.List;
import java.util.Locale;

import org.bukkit.command.Command;
import org.bukkit.command.CommandSender;
import org.bukkit.command.TabCompleter;

public class DeltaTabCompleter implements TabCompleter {
    private final Delta plugin;
    public DeltaTabCompleter(Delta plugin) {
        this.plugin = plugin;
    }

    @Override
    public List<String> onTabComplete(CommandSender sender, Command command, String label,String[] args){
        List<String> suggestions = new ArrayList<>();

        if (args.length  == 1) {
            List<String> commands = List.of("help", "initialize", "save", "restore", "list", "select", "status", "selected");
            for (String cmd : commands) {
                if (cmd.startsWith(args[0].toLowerCase())) {
                    suggestions.add(cmd);
                }
            }
            return suggestions;
        }
        if (args.length == 2) {
            switch(args[0].toLowerCase()) {
                case "list":
                    for (String s : List.of("projects", "commits")) {
                        if (s.startsWith(args[1].toLowerCase())) suggestions.add(s);
                    }
                    return suggestions;
                case "help":
                    for (String s : List.of("help", "initialize", "save", "restore", "list", "select", "status", "selected")) {
                        if (s.startsWith(args[0].toLowerCase())) suggestions.add(s);
                    }
                    return suggestions;
                default:
                    break;
            }
            return suggestions;
        }
        return suggestions;
    }
}
